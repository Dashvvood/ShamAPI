...
from model.qwen3_vl import chat as model_chat
from model.qwen3_vl import load_qwen3_vl, openai_messages_to_qwen
from utils.api_process import get_client_info

_CONFIG_NAME = os.environ.get("CONFIG", "dev").lower()
_CONFIG_PATH = PROJECT_ROOT / "config" / _CONFIG_NAME / "qwen3_vl.yaml"
_CONFIG = load_config(str(_CONFIG_PATH))


def _load() -> AppState:
    setup_logger(_CONFIG)
    logger.info("Loading Qwen3-VL from {}", _CONFIG["model"]["model_dir"])
...
def chat(
    ...
    """Native multimodal chat (image / video / text)."""
...
def openai_chat_completions(
    ...
    """OpenAI-compatible; supports image_url / video_url content parts."""
    if body.stream:
        raise api_error(
            "/v1/chat/completions",
    ...
    messages = openai_messages_to_qwen(
